...
class Config extends EventEmitter {
  ...
  loadConfigs() {
    ...
    try {
      ...
      if (!fs.existsSync(this.userConfigFilePath) && this.isUserConfigDirEmpty(true)) {
        console.log(`用户配置文件 ${this.userConfigFilePath} 不存在且目录为空，将从默认配置初始化。`);
      ...
      if (!this.validateConfig(this.config)) { // Validate the newly merged config
        ...
        // console.log("配置已成功加载并验证:", JSON.stringify(this.config, null, 2));
        console.log("配置已成功加载并验证。");
      }
  ...
  copyDefaultConfigsToUserDir() {
    ...
    if (fs.existsSync(defaultFileSourcePath)) {
        try {
            ...
            console.log(`已将默认配置文件 ${defaultFileSourcePath} 复制到用户配置文件 ${userFileDestPath}`);
        } catch (copyError) {
  ...
  onFileChange(filePath, event) {
    console.log(`配置文件 ${event}: ${filePath}。重新加载所有配置...`);
    this.loadConfigs();
  }
  ...
  saveUserConfig(userConfigData) {
    try {
      ...
      console.log(`用户配置已保存到: ${this.userConfigFilePath}`);
    } catch (error) {
...
const configProxy = new Proxy(configInstance, {
  ...
  set(target, prop, value) {
    if (prop in target.config) {
        // console.warn(`警告: 不建议直接通过代理设置配置属性 '${String(prop)}'。`);
        return false;
    }
    target[prop] = value;
    return true;
